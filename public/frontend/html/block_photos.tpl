...
<div  class="photo-container pop-photos">
  ...
  <div class="hov" id="photo-block-${ID_PHOTO}"> <a class='photo_big' title="${DESCRIPTION}" href="${BIG_IMAGE}" data-lightbox="roadtrip" data-num='${ID_PHOTO}'> <img src="${SMALL_IMAGE}" alt="">
	...
	<!-- IF '${ALLOW_EDIT}' != '' --><span class="icons-hid"><i id="my-video-${ID_PHOTO}" class="remove_pic" id="${ID_PHOTO}" data-item="${ID_PHOTO}" data-tooltip="${STR_REMOVE_PHOTO}"><img src="./frontend/images/icon-krest.png" alt=""></i></span><!-- END IF --> 
	</span> 
	</div>
  <!-- END IF -->
  <!-- END row_photos_list -->
...
$(document).scroll(function() {
	...
	if($(window).scrollTop()+$(window).height()>=$(document).height()){
		if (evJob) {
			evJob = false;
			$('.photo-container').append('<div class="loading-bar"><img border="0" src="./frontend/images/select2-spinner.gif" width=20px></div>')
			$.ajax({
				type:'POST',
