...
$(document).ready(function () {
    ...
    */
    ...
    /*REMOVE DIALOG*/
    $(document).on('click', '.del-dialog', function (event) {
        event.preventDefault();
    ...
    $(document).on("click", "#send-reply", function () {
        ...
        if (comment != '' || attach.length != 0) {
            $('#my-comment-' + IdComment).append('<div class="loading-mess"><img border="0" src="./frontend/images/select2-spinner.gif" width=20px></div>');
            const formData = then.serializeArray();
            formData.push({'name': 'attach', 'value': attach});
            if (csrfToken()) {
                formData.push({'name': '_token', 'value': csrfToken()});
